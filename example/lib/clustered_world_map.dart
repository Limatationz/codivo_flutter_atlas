...
class ClusteredWorldMap extends StatefulWidget {
  const ClusteredWorldMap({Key? key}) : super(key: key);

  @override
  State<ClusteredWorldMap> createState() => _ClusteredWorldMapState();
}

class _ClusteredWorldMapState extends State<ClusteredWorldMap> {
  final providerSupportsSatellite =
      AtlasProvider.instance!.supportedMapTypes!.length > 1;
  var _mapType = MapType.normal;

  var _currentCameraPosition = initialCameraPosition;
    ...
    return Stack(children: [
      Atlas(
        ...
        onCameraPositionChanged: (newCameraPosition) =>
        {
          ...
          providerSupportsSatellite ? _controllerSmall.moveCamera(
      ...
      Positioned(
        ...
        child: IconButton(
          ...
          onPressed: () async {
            _controllerLarge.moveCamera(initialCameraPosition);
          },
        ),
      ),
    ]);
